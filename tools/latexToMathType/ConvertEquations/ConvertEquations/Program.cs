...
namespace ConvertEquations
{
    class Program
    {
        ...
        {
            // CLI 模式：支持本地直接调用，不依赖 RocketMQ
            // 示例：
            //   ConvertEquations.exe --latex "$\\frac{x^2+5}{y^2-4}=1$"
            if (args != null && args.Length > 0 && HasArg(args, "--latex"))
            {
                var latexRaw = GetArgValue(args, "--latex");
                if (string.IsNullOrWhiteSpace(latexRaw))
                {
                    Console.Error.WriteLine("missing_latex_argument");
                    Environment.Exit(2);
                }
                try
                {
                    ...
                    {
                        Console.Error.WriteLine("latex_to_mathtype_failed");
                        Environment.Exit(10);
                    }
                ...
                {
                    Console.Error.WriteLine("cli_exception:" + e.Message);
                    Environment.Exit(20);
                }
                finally
                {
                    try { OLE_WMF.DeInitWord(); } catch { }
                }
            }
